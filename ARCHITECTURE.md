...
파일 쓰기 전용 API와 표준 `file_write`/`file_edit`/`file_delete`/`file_move`/`file_apply_patch` tool은 실행 전에 `.kkode/checkpoints` file snapshot을 만들고 `checkpoint_id`를 반환해요. `POST /api/v1/files/restore`와 `file_restore_checkpoint` tool은 같은 checkpoint payload를 복구해서 외부 adapter가 권한 프롬프트 없이도 undo UI를 붙일 수 있게 해요.
