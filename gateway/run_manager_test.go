...
func TestAsyncRunManagerPersistsRunState(t *testing.T) {
	...
	run, err := manager.Start(ctx, RunStartRequest{SessionID: sess.ID, Prompt: "go", Provider: "copilot", Model: "gpt-5-mini", WorkingDirectory: "services/api", MCPServers: []string{"mcp_1"}, Skills: []string{"skill_1"}, Subagents: []string{"agent_1"}, EnabledTools: []string{"file_read"}, DisabledTools: []string{"shell_run"}, ContextBlocks: []string{"adapter token=ghp_123456789012345678901234567890123456 context"}})
	if err != nil {
		t.Fatal(err)
	}
	waitForRunStatus(t, manager, run.ID, "completed")
	...
	if got.Status != "completed" || got.TurnID != "turn_1" || got.Provider != "copilot" || got.Model != "gpt-5-mini" || got.WorkingDirectory != "services/api" || len(got.MCPServers) != 1 || got.MCPServers[0] != "mcp_1" || len(got.EnabledTools) != 1 || got.EnabledTools[0] != "file_read" || len(got.DisabledTools) != 1 || got.DisabledTools[0] != "shell_run" || len(got.ContextBlocks) != 1 || strings.Contains(got.ContextBlocks[0], "ghp_") || !strings.Contains(got.ContextBlocks[0], "[REDACTED]") {
