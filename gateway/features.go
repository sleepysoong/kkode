...
func DefaultFeatureCatalog() []FeatureDTO {
	return []FeatureDTO{
		...
		{Name: "background_runs", Status: "implemented", Description: "run을 즉시 접수하고 background 상태 조회, 취소, live SSE event를 제공해요.", Endpoints: []string{"GET /api/v1/runs", "POST /api/v1/runs", "GET /api/v1/runs/{run_id}", "GET /api/v1/runs/{run_id}/events", "POST /api/v1/runs/{run_id}/cancel"}},
