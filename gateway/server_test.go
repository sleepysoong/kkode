...
func TestGatewayCapabilitiesDiscovery(t *testing.T) {
	...
	if caps.Version != "test" || len(caps.Providers) != 1 || len(caps.Features) == 0 || len(caps.DefaultMCPServers) != 1 || caps.Limits.MaxRequestBytes != 1234 || caps.Limits.MaxConcurrentRuns != 3 || caps.Limits.RunTimeoutSeconds != 120 || caps.Limits.RunMaxIterations != 8 || caps.Limits.RunWebMaxBytes != 1<<20 || caps.Limits.MaxSessionIDBytes != maxSessionIDBytes || caps.Limits.MaxTurnIDBytes != maxRunIDBytes || caps.Limits.MaxRunIDBytes != maxRunIDBytes || caps.Limits.MaxProjectRootBytes != maxProjectRootBytes || caps.Limits.MaxMCPHTTPResponseBytes != maxMCPHTTPResponseBytes || caps.Limits.MaxMCPProbeNameBytes != maxMCPProbeNameBytes || caps.Limits.MaxMCPProbeURIBytes != maxMCPProbeURIBytes || caps.Limits.MaxMCPProbeArgumentBytes != maxMCPProbeArgumentsBytes || caps.Limits.MaxMCPProbeOutputBytes != maxMCPProbeOutputBytes || caps.Limits.MaxFileContentBytes != maxFileContentBytes || caps.Limits.MaxFilePathBytes != maxFilePathBytes || caps.Limits.MaxFilePatternBytes != maxFilePatternBytes || caps.Limits.MaxFileCheckpointIDBytes != maxFileCheckpointIDBytes || caps.Limits.MaxCheckpointIDBytes != maxCheckpointIDBytes || caps.Limits.MaxResourceIDBytes != maxResourceIDBytes || caps.Limits.MaxArtifactIDBytes != maxArtifactIDBytes || caps.Limits.MaxTodoIDBytes != maxTodoIDBytes || caps.Limits.MaxSkillPreviewBytes != maxSkillPreviewBytes || caps.Limits.MaxSubagentPreviewBytes != maxSubagentPreviewPromptBytes || caps.Limits.MaxPromptTemplateNameBytes != maxPromptTemplateNameBytes || caps.Limits.MaxPromptTextBytes != maxPromptTextBytes || caps.Limits.MaxTranscriptMarkdownBytes != maxTranscriptMarkdownBytes || caps.Limits.MaxGitDiffBytes != maxGitDiffBytes || caps.Limits.MaxGitPathBytes != maxGitPathBytes || caps.Limits.MaxRunPreviewBytes != MaxRunPreviewBytes || caps.Limits.MaxRunOutputTokens != MaxRunOutputTokens || caps.Limits.MaxProviderTestPreviewBytes != MaxProviderTestPreviewBytes || caps.Limits.MaxProviderTestResultBytes != MaxProviderTestResultBytes || caps.Limits.MaxProviderTestOutputTokens != MaxProviderTestOutputTokens || caps.Limits.MaxProviderTestTimeoutMS != MaxProviderTestTimeoutMS || caps.Limits.MaxHTTPJSONResponseBytes != httpjson.MaxResponseBytes || caps.Limits.MaxWorkspaceFileReadBytes != workspace.MaxFileReadBytes || caps.Limits.MaxWorkspaceFileWriteBytes != workspace.MaxFileWriteBytes || caps.Limits.MaxWorkspaceListEntries != workspace.MaxListEntries || caps.Limits.MaxWorkspaceGlobMatches != workspace.MaxGlobMatches || caps.Limits.MaxWorkspaceGrepMatches != workspace.MaxGrepMatches || caps.Limits.MaxWorkspacePatchBytes != workspace.MaxPatchBytes || caps.Limits.MaxLSPFormatInputBytes != maxLSPFormatInputBytes || caps.Limits.MaxLSPFormatPreviewBytes != maxLSPFormatPreviewBytes || caps.Limits.MaxRunPromptBytes != maxRunPromptBytes || caps.Limits.MaxRunSelectorItems != maxRunSelectorItems || caps.Limits.MaxRunContextBlocks != maxRunContextBlocks || caps.Limits.MaxRequestIDBytes != maxRequestIDBytes || caps.Limits.MaxIdempotencyKeyBytes != maxIdempotencyKeyBytes || caps.Limits.MaxToolCallNameBytes != maxToolCallNameBytes || caps.Limits.MaxToolCallIDBytes != maxToolCallIDBytes || caps.Limits.MaxToolCallArgumentBytes != maxToolCallArgumentsBytes || caps.Limits.MaxToolCallOutputBytes != maxToolCallOutputBytes || caps.Limits.MaxToolCallWebBytes != maxToolCallWebBytes || caps.Limits.MaxShellTimeoutMS != workspace.MaxCommandTimeout.Milliseconds() || caps.Limits.MaxShellOutputBytes != workspace.MaxCommandOutputBytes || caps.Limits.MaxShellStderrBytes != workspace.MaxCommandStderrBytes {
...
func TestGatewaySessionTurnsAPI(t *testing.T) {
	...
	req = httptest.NewRequest(http.MethodGet, "/api/v1/sessions/"+sess.ID+"/turns/"+strings.Repeat("x", maxRunIDBytes+1), nil)
	rec = httptest.NewRecorder()
	srv.ServeHTTP(rec, req)
	if rec.Code != http.StatusBadRequest || !strings.Contains(rec.Body.String(), "turn_id") {
		t.Fatalf("긴 turn detail id는 400이어야 해요: status=%d body=%s", rec.Code, rec.Body.String())
	}
}
...
func TestGatewayPromptTemplateAPIs(t *testing.T) {
	...
	req = httptest.NewRequest(http.MethodGet, "/api/v1/prompts/"+strings.Repeat("x", maxPromptTemplateNameBytes+1), nil)
	rec = httptest.NewRecorder()
	srv.ServeHTTP(rec, req)
	if rec.Code != http.StatusBadRequest || !strings.Contains(rec.Body.String(), "prompt template") {
		t.Fatalf("긴 prompt template 이름은 400이어야 해요: status=%d body=%s", rec.Code, rec.Body.String())
	}
	...
	req = httptest.NewRequest(http.MethodPost, "/api/v1/prompts/"+strings.Repeat("x", maxPromptTemplateNameBytes+1)+"/render", body)
	req.Header.Set("Content-Type", "application/json")
	rec = httptest.NewRecorder()
	srv.ServeHTTP(rec, req)
	if rec.Code != http.StatusBadRequest || !strings.Contains(rec.Body.String(), "prompt template") {
		t.Fatalf("긴 prompt render template 이름은 400이어야 해요: status=%d body=%s", rec.Code, rec.Body.String())
	}
}
...
func TestGatewayResourceManifestLifecycle(t *testing.T) {
	...
	longResourceID := strings.Repeat("x", maxResourceIDBytes+1)
	for _, tc := range []struct {
		name   string
		method string
		path   string
		...
		{name: "detail", method: http.MethodGet, path: "/api/v1/mcp/servers/" + longResourceID},
		{name: "update", method: http.MethodPut, path: "/api/v1/mcp/servers/" + longResourceID, body: `{"name":"filesystem","config":{"kind":"http","url":"https://mcp.example.test"}}`},
		{name: "delete", method: http.MethodDelete, path: "/api/v1/mcp/servers/" + longResourceID},
		{name: "skill preview", method: http.MethodGet, path: "/api/v1/skills/" + longResourceID + "/preview"},
	} {
		...
		srv.ServeHTTP(rec, req)
		if rec.Code != http.StatusBadRequest || !strings.Contains(rec.Body.String(), "resource id") {
			t.Fatalf("긴 resource id %s는 400이어야 해요: status=%d body=%s", tc.name, rec.Code, rec.Body.String())
		}
	}
...
func TestGatewayGitStatusDiffAndLog(t *testing.T) {
	...
	req = httptest.NewRequest(http.MethodGet, "/api/v1/git/diff?project_root="+url.QueryEscape(root)+"&path="+url.QueryEscape(strings.Repeat("x", maxGitPathBytes+1)), nil)
	rec = httptest.NewRecorder()
	srv.ServeHTTP(rec, req)
	if rec.Code != http.StatusBadRequest || !strings.Contains(rec.Body.String(), "path") {
		t.Fatalf("긴 git diff path는 400이어야 해요: status=%d body=%s", rec.Code, rec.Body.String())
	}
...
func TestGatewayMutatesSessionTodos(t *testing.T) {
	...
	req = httptest.NewRequest(http.MethodDelete, "/api/v1/sessions/"+sess.ID+"/todos/"+strings.Repeat("x", maxTodoIDBytes+1), nil)
	rec = httptest.NewRecorder()
	srv.ServeHTTP(rec, req)
	if rec.Code != http.StatusBadRequest || !strings.Contains(rec.Body.String(), "todo id") {
		t.Fatalf("긴 todo delete id는 400이어야 해요: status=%d body=%s", rec.Code, rec.Body.String())
	}
...
func TestGatewayCreatesAndReadsCheckpoints(t *testing.T) {
	...
	req = httptest.NewRequest(http.MethodGet, "/api/v1/sessions/"+sess.ID+"/checkpoints/"+strings.Repeat("x", maxCheckpointIDBytes+1), nil)
	rec = httptest.NewRecorder()
	srv.ServeHTTP(rec, req)
	if rec.Code != http.StatusBadRequest || !strings.Contains(rec.Body.String(), "checkpoint id") {
		t.Fatalf("긴 checkpoint detail id는 400이어야 해요: status=%d body=%s", rec.Code, rec.Body.String())
	}
}
...
func TestGatewayCreatesListsReadsAndDeletesArtifacts(t *testing.T) {
	...
	longArtifactID := strings.Repeat("x", maxArtifactIDBytes+1)
	for _, tc := range []struct {
		name   string
		method string
		path   string
	}{
		{name: "detail", method: http.MethodGet, path: "/api/v1/artifacts/" + longArtifactID},
		{name: "delete", method: http.MethodDelete, path: "/api/v1/artifacts/" + longArtifactID},
	} {
		req = httptest.NewRequest(tc.method, tc.path, nil)
		rec = httptest.NewRecorder()
		srv.ServeHTTP(rec, req)
		if rec.Code != http.StatusBadRequest || !strings.Contains(rec.Body.String(), "artifact id") {
			t.Fatalf("긴 artifact id %s는 400이어야 해요: status=%d body=%s", tc.name, rec.Code, rec.Body.String())
		}
	}
