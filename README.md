...
- `GET /healthz`, `GET /readyz`, `GET /api/v1`, `GET /api/v1/openapi.yaml`, `GET /api/v1/version`, `GET /api/v1/capabilities`, `GET /api/v1/diagnostics`, `GET /api/v1/providers`, `GET /api/v1/providers/{provider}`, `GET /api/v1/models`, `GET /api/v1/stats`를 제공해요.
...
- `GET /api/v1`은 외부 adapter가 health/readiness/OpenAPI/capabilities/session/run/transcript/event/preview 같은 대표 link를 한 번에 발견하게 해요. `KKODE_CORS_ORIGINS` 또는 `-cors-origins`를 지정하면 별도 웹 패널 origin에서 bearer auth API를 호출할 수 있어요. 모든 gateway 응답은 `X-Request-Id`를 보존하거나 생성하고, 실패 응답은 공개 DTO인 `ErrorEnvelope{error:{code,message,request_id,details}}` 형태로 반환해서 웹 패널/Discord 로그와 오류 처리를 같은 요청으로 묶을 수 있게 해요. host app은 `gateway.AccessLogger`를 주입해서 같은 request id, method, path, status, byte 수, duration을 structured log나 metric으로 받을 수 있고, `POST /api/v1/runs/preview`는 실행 없이 provider/model/default MCP/선택 manifest/base request tool 조립 결과와 provider API/source 호출 직전 변환 preview를 보여주고, `preview_stream=true`면 streaming payload도 같은 변환 레이어로 미리 보여주며, body/raw preview는 secret 마스킹과 길이 제한을 적용해요. `POST /api/v1/runs`와 retry 요청은 같은 값을 run metadata의 `request_id`로 남겨요. `GET /api/v1/openapi.yaml`은 외부 adapter와 SDK generator가 현재 API 계약을 내려받게 해요. `GET /api/v1/capabilities`는 sessions/events/todos/background_runs/models/prompts/MCP/skills/subagents/LSP의 현재 지원 상태, 기본 Serena/Context7 MCP manifest, `limits.max_request_bytes`, `limits.max_concurrent_runs`, `limits.run_timeout_seconds`를 외부 adapter가 발견할 수 있게 해요. `GET /api/v1/diagnostics`는 store ping, run starter/previewer/validator 연결, provider/default MCP 개수, Serena/Context7 기본 MCP 상태, 동시 run 제한, run timeout, 현재 queued/running/cancelling queue 상태 같은 배포 진단값을 한 번에 보여줘요. default MCP discovery/preview 응답은 header/env secret 값을 마스킹해요. `GET /api/v1/stats`는 dashboard가 sessions/turns/events/runs/resources 카운트와 `total_runs`, `total_resources`를 한 번에 그리게 해요.
- `GET /api/v1/providers`, `GET /api/v1/providers/{provider}`, `GET /api/v1/models`는 provider alias, 모델 catalog, 기본 모델, capability, auth 상태, auth env 힌트, converter/caller/source/operation/route 변환 profile을 반환해서 외부 adapter가 모델 선택 UI와 provider debug 화면을 만들게 해요. `provider` path/query 값은 canonical 이름과 alias를 모두 허용해요.
