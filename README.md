...
- `GET /api/v1/stats`의 `sessions_by_provider`, `sessions_by_model`, `sessions_by_mode`, `events_by_type`, `todos_by_status`, `artifacts_by_kind`, `artifact_bytes`, `artifact_bytes_by_kind`, `run_events`, `run_events_by_type`, `run_duration`, `run_duration_by_provider`, `run_duration_by_model`, `run_usage`, `run_usage_by_provider`, `run_usage_by_model`은 SQLite에 저장된 session/event/todo/artifact/background run event/timestamp/usage를 합산해서 dashboard가 provider/model/mode, event type, todo status, artifact kind와 저장량, replay 규모, 평균/최대/p95 latency, token 합계를 계산하려고 목록 전체를 다시 훑지 않게 해요.
