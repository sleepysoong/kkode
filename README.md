...
- `GET /api/v1/stats`의 `run_events`, `run_duration`, `run_duration_by_provider`, `run_duration_by_model`, `run_usage`, `run_usage_by_provider`, `run_usage_by_model`은 SQLite에 저장된 background run event/timestamp/usage를 합산해서 dashboard가 replay 규모, 평균/최대/p95 latency, token 합계를 계산하려고 run 목록 전체를 다시 훑지 않게 해요.
