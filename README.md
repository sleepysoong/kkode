...
- `GET /api/v1/providers`, `GET /api/v1/providers/{provider}`, `GET /api/v1/models`는 provider alias, 모델 catalog, 기본 모델, capability, auth 상태, 변환 profile을 반환해서 외부 adapter가 모델 선택 UI와 provider debug 화면을 만들게 해요. `provider` path/query 값은 canonical 이름과 alias를 모두 허용해요.
