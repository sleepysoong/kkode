...
- Capabilities discovery는 `limits.max_subagent_preview_prompt_bytes`, `limits.max_prompt_text_bytes`, `limits.max_transcript_markdown_bytes`, `limits.max_git_diff_bytes`, `limits.max_git_path_bytes`도 노출해서 subagent/prompt/transcript/git preview 요청을 실행 전에 bounded 값으로 맞추게 해요.
