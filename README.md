...
- `GET /api/v1/files`, `GET/PUT /api/v1/files/content`, `POST /api/v1/files/patch`, `GET /api/v1/files/glob`, `GET /api/v1/files/grep`는 웹 패널 파일 브라우저용 목록/읽기/쓰기/patch/glob/검색 API예요. 파일 content 응답은 `content_bytes`, `file_bytes`, `content_truncated`를 포함해 대용량 preview 여부를 표시하고, glob 응답은 `total_paths`, `limit`, `paths_truncated`로 더 많은 결과가 있는지 알려주며, 내부적으로 workspace 경계를 재사용해요.
