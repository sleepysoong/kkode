...
func TestSQLiteStoreSessionLifecycle(t *testing.T) {
	...
	filtered, err := store.ListSessions(ctx, SessionQuery{ProviderName: "openai", Model: "gpt-5-mini", Mode: AgentModeBuild, Limit: 10})
	if err != nil {
		t.Fatal(err)
	}
...
func TestSQLiteStoreLoadsDashboardStats(t *testing.T) {
	...
	if stats.Runs["completed"] != 2 || stats.RunsByProvider["copilot"] != 1 || stats.RunsByProvider["openai"] != 1 || stats.RunsByModel["gpt-5-mini"] != 2 || stats.Resources[string(ResourceSkill)] != 1 || stats.ResourcesByEnabled["disabled"] != 1 {
...
func TestRunStorePersistsBackgroundRuns(t *testing.T) {
	...
	byTurnID, err := store.ListRuns(ctx, RunQuery{SessionID: sess.ID, TurnID: "turn_1", Limit: 10})
	if err != nil {
		t.Fatal(err)
	}
	if len(byTurnID) != 1 || byTurnID[0].ID != "run_1" || byTurnID[0].TurnID != "turn_1" {
		t.Fatalf("turn_id run list가 이상해요: %+v", byTurnID)
	}
	assertSQLiteIndexExists(t, store, "runs", "idx_runs_turn_updated")
