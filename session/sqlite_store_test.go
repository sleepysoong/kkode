...
func TestRunStorePersistsBackgroundRuns(t *testing.T) {
	...
	saved, err := store.SaveRun(ctx, Run{ID: "run_1", SessionID: sess.ID, Status: "queued", Prompt: "go", Provider: "copilot", Model: "gpt-5-mini", MCPServers: []string{"mcp_1"}, Skills: []string{"skill_1"}, Subagents: []string{"agent_1"}, EnabledTools: []string{"file_read"}, DisabledTools: []string{"shell_run"}, EventsURL: "/api/v1/sessions/" + sess.ID + "/events", Metadata: map[string]string{"source": "test", "request_id": "req_store"}})
	if err != nil {
		t.Fatal(err)
	}
