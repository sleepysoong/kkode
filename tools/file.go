...
func FileTools(ws *workspace.Workspace) ([]llm.Tool, llm.ToolRegistry) {
	...
	defs := []llm.Tool{
		...
		{Kind: llm.ToolFunction, Name: "shell_run", Description: "workspace command를 실행하고 구조화 결과를 돌려줘요", Strict: &strict, Parameters: objectSchemaRequired(map[string]any{"command": stringSchema(), "args": arraySchema(stringSchema()), "timeout_ms": integerSchema()}, []string{"command"})},
	}
