...
|---|---|---|---:|---|
| Agent loop/session | SQLite session resume/fork, todo, compaction, artifact API/export/import, direct tool output artifact promotion, background run store, run SSE | interrupt queue, agent-run artifact promotion policy, checkpoint rewind, cost budget | P0 | `01-agent-loop-session-state.md` |
...
`file_write`, `file_edit`, `file_delete`, `file_move`, `file_apply_patch`, 전용 files API write/delete/move/patch는 실행 전 `.kkode/checkpoints` snapshot을 만들고 `file_restore_checkpoint`와 `POST /api/v1/files/restore`로 복구할 수 있어요. 그래도 Claude Code처럼 `/undo`, `/redo`, `/rewind` UX와 코드/대화 복구를 분리하는 흐름은 아직 부족해요.

남은 것은 다음이에요.

- `/undo`, `/redo`, `/rewind` command
- conversation-only rewind와 code restore를 묶는 UX
- checkpoint list/cleanup/retention 정책
...
2. YOLO 실행 경계 hardening + file mutation checkpoint/restore UX polish
