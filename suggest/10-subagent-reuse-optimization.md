...
6. Gateway request id 처리를 middleware로 공통화했어요.
   ...
   - `cmd/kkode-gateway`의 `-access-log`/`KKODE_ACCESS_LOG=1`이 이 hook을 JSONL stderr 출력으로 연결해서 컨테이너/VM 배포에서도 바로 수집할 수 있게 했고, SIGINT/SIGTERM graceful shutdown, background run shutdown cancel, `/readyz` store ping, configurable request body limit, chunked oversized JSON 413 처리, HTTP lifecycle timeout flag/env도 추가했어요.
