...
6. Gateway request id 처리를 middleware로 공통화했어요.
   ...
   - `GET /api/v1/runs?request_id=...` 필터, `GET /api/v1/requests/{request_id}/runs`, `GET /api/v1/requests/{request_id}/events` correlation API를 추가해서 외부 adapter가 HTTP 요청에서 파생된 background run과 event replay를 다시 찾을 수 있게 했고, SQLite에는 `idx_runs_request_id_updated` expression index를 추가했어요.
